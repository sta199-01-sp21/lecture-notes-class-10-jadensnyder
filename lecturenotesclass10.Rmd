...
```{r packages}
library(tidyverse)
install.packages(vcd) # used for Arthritis data
...
These values are very close to the "true" probabilities. The calculated 
probability of drawing a blue is 37.5%, and the probability of drawing a white 
is 12.5%. The probability of drawing a red is 50%. Each of these values is 
within 5% of the experiment results, so they are very close.
...
```{r simulate_rolls}
roll_results <- tibble(die_1 = replication(n = 10000, expr = sample(1:6,
  size = 1)), die_2 = replication(n = 10000, expr = sample(1:6, size = 1)))
```
